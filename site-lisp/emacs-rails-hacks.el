...
(defadvice rails-core:prepare-command (around rails-hacks--zeus-maybe (command) activate)
  (if (and
       (not (file-exists-p (rails-core:file ".noz")))
       (or (file-exists-p (rails-core:file ".zeus.sock"))
           (file-exists-p (rails-core:file "zeus.json"))
           (file-exists-p (rails-core:file "tmp/spring/spring"))
           (file-exists-p (rails-core:file "config/spring.rb"))))
      (setq ad-return-value (concat "z-maybe " command))
    ad-do-it))

(defun rails-hacks--bindings ()
  ;; use rspec mode to run test
  (define-key rails-minor-mode-map (kbd "C-c , , #") nil)
  (define-key rails-minor-mode-map (kbd "C-c , , .") nil)
  (define-key rails-minor-mode-map (kbd "C-c , , /") nil)
  (define-key rails-minor-mode-map (kbd "C-c , , z") nil)

  (define-key rails-minor-mode-map (kbd "C-c , , SPC")  'rails-lib:run-primary-switch)
  (define-key rails-minor-mode-map (kbd "C-c , , <return>")  'rails-lib:run-secondary-switch)
  (define-key rails-minor-mode-map (kbd "C-c , , g j")  'rails-nav:goto-assets-javascripts)
  (define-key rails-minor-mode-map (kbd "C-c , , g s")  'rails-nav:goto-assets-stylesheets)
  (define-key rails-minor-mode-map (kbd "C-c , , f j")  'rails-find:assets-javascripts)
  (define-key rails-minor-mode-map (kbd "C-c , , f s")  'rails-find:assets-stylesheets)
